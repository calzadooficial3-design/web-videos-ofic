...
-- save_admin_snapshot usaba pg_advisory_xact_lock, que ESPERA de forma
-- indefinida hasta poder tomar el candado. El problema: si una sola
-- conexion queda en mal estado sosteniendolo (por una desconexion a medio
-- guardado, un problema de red entre PostgREST y Postgres, etc.), TODAS las
-- demas conexiones que intentan guardar se quedan esperando para siempre
-- detras de ella, consumiendo una conexion cada una hasta agotar el pool
-- completo -y entonces hasta las lecturas mas simples, sin relacion alguna,
-- dejan de responder. Nos paso varias veces en la misma tarde.
--
-- El cambio: reintentar con pg_try_advisory_xact_lock (no bloqueante) hasta
-- 2 segundos en vez de esperar para siempre. Si no logra tomar el candado
-- en ese tiempo, falla con un mensaje claro (que el cliente ya interpreta
-- como "vuelve a intentar") en vez de quedarse colgado ocupando una
...
begin
  ...
  while not v_lock_acquired and v_lock_attempts < 20 loop
    v_lock_acquired := pg_catalog.pg_try_advisory_xact_lock(v_lock_key);
    if not v_lock_acquired then
      v_lock_attempts := v_lock_attempts + 1;
