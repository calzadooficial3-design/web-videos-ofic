...
-- La version anterior (pg_try_advisory_xact_lock en un bucle con pg_sleep
-- cada 100ms) se comporta mal bajo contencion real: todas las conexiones en
-- espera se despiertan a la vez en cada intento y compiten de nuevo por el
-- candado sin ningun orden, asi que con varias llamadas simultaneas es
-- posible que NINGUNA lo consiga dentro de sus 20 intentos (una estampida /
-- livelock), y mientras tanto cada una ocupa una conexion del pool. Eso es
-- justo lo que se vio en produccion: varias conexiones atascadas en
-- "PgSleep" al mismo tiempo, sin que ninguna avance.
--
-- La forma correcta en Postgres es dejar que el propio motor haga la espera
-- (usa una cola FIFO eficiente, sin sondeo) y ponerle un limite de tiempo
-- con lock_timeout: si no consigue el candado en 3 segundos, Postgres
-- cancela la espera solo y lanza lock_not_available, que se traduce al
-- mismo error SAVE_BUSY de antes.
